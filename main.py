...
config['train_path'] ="./data/{}/train.txt".format(config['dataset'])
config['val_path'] ="./data/{}/val.txt".format(config['dataset'])
config['test_path'] ="./data/{}/test.txt".format(config['dataset'])
config['case_path'] ="./data/{}/case.txt".format(config['dataset'])
    ...
    if config['run_type']=='train':
        ...
        for e in range(config['epoch']):
            ...
            if e%config['val_step']==0:
                ...
                with open('{}_i1_gcn1_gru2_aug2.txt'.format(config['dataset']),'a') as f:
                    for k in config['topk']:
                        start_time = time.time()
                        start_time_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(start_time))
                        res="{} : Epoch{} Val K={}  f1:{}  hr:{}\n".format(start_time_str,e,k,f1s[k]/sample_num,hits[k]/gts)
                        f.write(res)
                    f.write("\n")
                ...
                if f1s[config['val_k']]>best_val:
                    ...
                    count=0
                    torch.save(gclrec.state_dict(), '{}_i1_gcn1_gru2_aug2.pth'.format(config['dataset']))
        ...
        gclrec.load_state_dict(torch.load('{}_i1_gcn1_gru2_aug2.pth'.format(config['dataset'])))
        f1s={}
        hits={}
        gts=0
        sample_num=0
        ...
        with open('{}_i1_gcn1_gru2_aug2.txt'.format(config['dataset']),'a') as f:
            for k in config['topk']:
                start_time = time.time()
                start_time_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(start_time))
                res="{} : Test K={}  f1:{}  hr:{}\n".format(start_time_str,k,f1s[k]/sample_num,hits[k]/gts)
                f.write(res)
            f.write("\n")
